...
EdTech LMS Platform with AI Tutoring - a comprehensive learning management system with AI-powered personalized tutoring, course marketplace, and adaptive learning.
Website type: education
Industry: edtech
Features: AI-powered personalized tutor with natural language explanations, Course marketplace with instructor tools, Adaptive learning paths with spaced repetition, Video streaming with adaptive bitrate, Interactive coding exercises with auto-grading, Live virtual classrooms with whiteboard, Progress tracking with analytics dashboard, Certification and badges, Discussion forums with AI moderation, Mobile app with offline sync, SCORM/xAPI compliance, Multi-language support, Dark mode, Instructor dashboard with revenue analytics.
Target: Students, educators, and institutions. Modern clean design with bento grid layouts, professional branding, international ready, WCAG AA compliant.

**Type:** Education  |  **Audience:** agencies, founders, and digital-product buyers  |  **Quality bar:** marketplace / ThemeForest grade
...
| Layer | Technology |
|-------|------------|
| Frontend | Next.js 14, React 18, TypeScript, Tailwind CSS, WebRTC (Live Classroom), Monaco Editor (Interactive Coding), Framer Motion |
| Backend | Node.js (NestJS), Python (FastAPI For AI Service Integration), WebSockets, GraphQL, Redis (Job Queue) |
| Database | PostgreSQL (Primary Data), Pinecone (Vector DB For AI Embeddings), Redis (Caching & Spaced Repetition State) |
...
Client-only static showcase (Mode 1) — `Education` landing experience with zero server hosting maintenance. The bundle is an API-less single-page app ready for direct drop into **Vercel** or **Netlify**.
